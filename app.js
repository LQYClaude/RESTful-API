			...
			if(result.rows[0]!=null){
				...
				while(result.rows[i]!=null){
					if(result.rows[i].tl<(new Date())){
						db.query('update task set state_id=1,modify_time=CURRENT_TIMESTAMP where task_id=$1', [result.rows[i].task_id]).then(result=>{
							console.log(' Change task state')
						}).catch(err => {
							console.error(' Unknow error.\n',err)
						})
						db.query('update assignment set delete_time=CURRENT_TIMESTAMP where task_id=$1 AND delete_time IS NULL', [result.rows[i].task_id]).then(result=>{
				...
				if(result.rows[0] == null){
					...
					db.query('SELECT * FROM task where task_id=$1',[result.rows[0].task_id]).then(result=>{
						if(result.rows[0].state_id != 2){
							return response(db,res,undefine,403,'Your assignment has already been accepted by you.',undefine,assignment_id,undefine,undefine,undefine)
						}else{
